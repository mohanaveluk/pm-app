...
import { MATERIAL_CATEGORY_CODE_PATTERN, MaterialCategory } from '../../models/material-category.model';
import { ConfirmDialogComponent } from '../../../../shared/components/confirm-dialog/confirm-dialog.component';

export interface MaterialCategoryFormDialogData {
  mode: 'create' | 'edit';
  materialCategoryId?: string;
}

export type MaterialCategoryFormDialogResult =
  | { action: 'saved'; materialCategory: MaterialCategory; saveAndNew: boolean }
  | undefined;

const NON_NEGATIVE_INTEGER_PATTERN = /^\d+$/;

@Component({
  selector: 'app-material-category-form-dialog',
  imports: [
    CommonModule, ReactiveFormsModule, MatDialogModule, MatFormFieldModule, MatInputModule,
    MatButtonModule, MatIconModule, MatSlideToggleModule, MatProgressSpinnerModule,
    MatDividerModule, MatTooltipModule,
  ],
  templateUrl: './material-category-form-dialog.component.html',
  styleUrl: './material-category-form-dialog.component.scss',
})
export class MaterialCategoryFormDialogComponent implements OnInit {
  private readonly fb = inject(FormBuilder);
  private readonly dialogRef = inject(MatDialogRef<MaterialCategoryFormDialogComponent, MaterialCategoryFormDialogResult>);
  private readonly dialog = inject(MatDialog);
  protected readonly store = inject(MaterialCategoryStore);
  protected readonly data = inject<MaterialCategoryFormDialogData>(MAT_DIALOG_DATA);
  ...
  private async loadMaterialCategory(id: string): Promise<void> {
    this.loading.set(true);
    this.loadError.set('');
    try {
      const category = await this.store.getMaterialCategoryById(id);
  ...
  async save(saveAndNew: boolean): Promise<void> {
    ...
    try {
      ...
      this.dialogRef.close({ action: 'saved', materialCategory, saveAndNew: false });
    } catch {
